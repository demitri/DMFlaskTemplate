...
url = 'https://content.sorghumbase.org/wordpress/index.php/wp-json/wp/v2/resource-link'
...
def test_resource_link_image_source(wp_api):
	'''
	Test that the link to the resource_image works properly.
	'''
	rl_request = ResourceLinkRequest(api=wp_api)
	rl_request.per_page = 1
	rls = rl_request.get()
	r = requests.get(rls[0].resource_image.s.guid)

	assert r.status_code == 200, "Resource image link is broken."
