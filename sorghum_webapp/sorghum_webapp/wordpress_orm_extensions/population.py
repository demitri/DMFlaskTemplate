...
class PopulationRequest(WPRequest):
	'''
	A class that encapsulates requests for WordPress populations.
	'''
	...
	def get(self, class_object=Population, count=False, embed=True):
		'''
		Returns a list of 'Population' objects that match the parameters set in this object.
		'''
		self.url = self.api.base_url + "population"
		...
		for d in links_data:
			...
			# Before we continue, do we have this Population in the cache already?
			try:
